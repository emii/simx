...
class Node(object):
    ...
    def __repr__(self):
        return '<%(cls)s - %(data)s>' % \
            dict(cls=self.__class__.__name__, data=repr((self.data,self.data[0],self.data[1])))
            #dict(cls=self.data.__class__.__name__, data=repr((self.data[0],self.data[1])))
...
class KDNode(Node):
    ...
    @require_axis
    def search_nn_p(self, point, best=None):
        """
        Search the nearest node of the given point
        ...
        while True:
            ...
            # consider the current node
            if current.dist(point) < best.dist(point) and current.dist(point) != 0:
                best = current
    ...
    @require_axis
    def search_nn_dist_p(self, point, distance):
        """
        Search the n nearest nodes of the given point which are within given
        distance
        ...
        # consider the current node
        if self.dist(point) < distance and self.dist(point) != 0:
            point.collide(self.data)
            point.attract(self.data,1)
            #best.append(self.data)
        ...
        for child, p in self.children:
            # check if child node needs to be recursed
            if self.axis_dist(point, self.axis) < math.pow(distance, 2):
                child.search_nn_dist_p(point, distance)
